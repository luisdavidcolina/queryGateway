...
async function CreaarReservaDetalle(ROOM_TYPE, reserva, precio, cliente, id_grupo, schema) {
  ...
  try {
    const impuestoQuery = `SELECT valor FROM $1.tbl_impuestos WHERE principal = true LIMIT 1`;
    const impuestoResult = await poolClient.query(impuestoQuery, [schema]);
    ...
    const habitacionQuery = `SELECT id FROM $1.tbl_habitaciones_tipo WHERE codigo = $2 LIMIT 1`;
    const habitacionResult = await poolClient.query(habitacionQuery, [schema, ROOM_TYPE.Type_Code]);
    if (!habitacionResult.rows.length) throw new Error("Tipo de habitación no encontrado");
    const id = habitacionResult.rows[0].id;

    const reservaGrupoQuery = `SELECT id FROM $1.tbl_reservas_grupo WHERE id_reservas = $2 LIMIT 1`;
    const reservaGrupoResult = await poolClient.query(reservaGrupoQuery, [schema, reserva.id]);
...
async function CrearReservaDetalles(id_grupos, ROOM_TYPE, id_reserva, cliente, schema) {
  ...
  try {
    ...
    const reservaQuery = `SELECT check_in_fecha, check_out_fecha FROM $1.tbl_reservas WHERE id = $2`;
    const reservaResult = await poolClient.query(reservaQuery, [schema, id_reserva]);
